...
async function loadConfig(path?: string) {
  ...
  try {
    ...
    return { json: JSON.parse(raw), path: candidate } as {
      json: any;
      path: string;
    };
  } catch {
...
(async () => {
  ...
  const { json: cfg, path } = await loadConfig(
  ...
  if (!apiKey || apiKey.length < 10) {
    console.error(
      "FACEIT_API_KEY missing or invalid. Add it to .env (git-ignored).",
    );
    process.exit(1);
  }
  ...
  if (!cfg["org-id"] && cfg["org-name"]) {
    try {
      ...
      console.warn(
        "Could not resolve org-id from org-name. Provide org-id explicitly.",
      );
    }
  }
  ...
  if (!cfg["champ-id"] && cfg["champ-name"] && cfg["org-id"]) {
    try {
      cfg["champ-id"] = await getChampionshipIdByName(
        cfg["org-id"],
        cfg["champ-name"],
        gameId,
      );
      changed = true;
      console.log(
        `Resolved champ-id: ${cfg["champ-id"]} (${cfg["champ-name"]})`,
      );
    } catch {
      console.warn(
        "Could not resolve champ-id from champ-name. Provide champ-id explicitly.",
      );
    }
  }
  ...
  if (!cfg["lb-id"] && cfg["champ-id"]) {
    try {
      const url = new URL(
        `${BASE}/leaderboards/championships/${cfg["champ-id"]}`,
      );
      ...
      if (cfg["lb-name"]) {
        chosen =
          items.find((it) => lbNameOf(it).trim() === cfg["lb-name"]) ||
          items.find((it) => lbNameOf(it).includes(cfg["lb-name"]));
      }
      if (!chosen && (cfg["lb-group"] || cfg["lb-group"] === 0)) {
        const g =
          typeof cfg["lb-group"] === "string"
            ? parseInt(cfg["lb-group"], 10)
            : cfg["lb-group"];
        if (!Number.isNaN(g as number))
          chosen = items.find((it) => it.group === g);
      }
      ...
      if (chosen?.leaderboard_id) {
        ...
        if (!cfg["lb-group"] && typeof chosen.group === "number")
          cfg["lb-group"] = chosen.group;
        changed = true;
        console.log(
          `Resolved lb-id: ${cfg["lb-id"]} (${lbNameOf(chosen) || "Group " + chosen.group})`,
        );
      } else {
        console.log(
          "Available leaderboards:",
          items.map((it) => ({
            id: it.leaderboard_id,
            group: it.group,
            name: lbNameOf(it),
          })),
        );
        console.warn(
          "Could not determine lb-id automatically. Provide lb-id or lb-name/lb-group/lb-pattern.",
        );
      }
    } catch {
      console.warn(
        "Failed to list championship leaderboards. Provide lb-id explicitly.",
      );
    }
  }
